...
async function loadTenant() {
  ...
  try {
    ...
    tenantInfo.value = resp || { tenantEnabled: false, voList: [] };
    // 选中第一个租户
    if (resp && resp.tenantEnabled && resp.voList && resp.voList.length > 0 && resp.voList[0]) {
      const firstTenantId = resp.voList[0].tenantId;
      loginFormRef.value?.getFormApi().setFieldValue('tenantId', firstTenantId);
    }
  } catch (err) {
    console.warn('当前后端未开启多租户或接口不存在，回退至单租户模式', err);
    tenantInfo.value = {
      tenantEnabled: false,
      voList: [],
    };
